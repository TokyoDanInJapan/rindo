...
/// that flutter_map cannot quietly turn them into blank tiles.
///
/// flutter_map's default tile provider awaits `httpClient.send()`, and reads
/// the body, with no timeout. A request that stalls on a half-open socket
/// therefore hangs *forever*. Such a stall is common on mobile after a network
/// handover or doze. A hung tile never fires `errorTileCallback`, so none of
/// the screen's recovery paths can even see it: not the offline banner, not
/// the 'tap to retry' placeholders, and not the re-key on the next JMA
/// success. The tile just pulses until the whole process is killed. That is
/// the 'base map goes blank, only a full restart fixes it' failure.
///
...
///
/// The connection cap and the socket-error retry exist for the zoom-out burst.
/// A pinch from z10 out to z6 re-keys all six radar frame layers at every even
/// zoom crossing. That lands a few hundred tile requests on this client within
/// a second. dart:io's default HttpClient has no per-host connection limit, so
/// each request opens its own socket. On a marginal mobile link the connect
/// storm itself then starts failing with SocketException 'No route to host'
/// (errno 113), while requests on already-warm connections keep succeeding.
/// A per-host cap queues the burst onto a few keep-alive sockets instead.
/// Retrying socket-level errors absorbs the stragglers, which is safe because
/// tile fetches are idempotent GETs. A transient connect failure therefore no
/// longer becomes a dead tile that the user has to tap to revive.
///
/// The outermost wrapper re-types whatever is left over. See
/// [TileFetchException] for why a dropped connection would otherwise be read
/// as 'this tile is empty'.
///
...
Client tileHttpClient({Client? transport}) {
  final inner =
      ...
      IOClient(
        HttpClient()
          // Small per-host cap: enough parallelism for tile bursts, without
          // the connect storm this cap exists to prevent. The radar sizes it.
          // The whole visible frame, about 100 tiny tiles, lands on
          // www.jma.go.jp at once. The other frames mount one at a time, see
          // radar_map_view.dart. Queue wait counts against the deadline below,
          // so the queue must drain a full frame within that deadline even on
          // a slow link. A cap of 6 proved too narrow, with tail-of-queue
          // timeouts. A cap of 8 clears it while staying near browser
...
/// the one case it was written for: the HTTP client closing while the map is
/// torn down. But dart:io words ordinary mid-request network loss the same
/// way, as 'Connection closed before full header was received' or 'Connection
/// closed while receiving data'. That is exactly what a mobile link does when
/// it drops a handover or wakes from doze.
///
/// A tile that 'succeeded' as blank is unrecoverable. `TileImage.loadError`
/// stays false, so `errorTileCallback` never fires. That leaves no failed-tile
/// banner, no tappable placeholder, and nothing to tell ConnectivityMonitor
/// that an outage is underway. TileImageManager will not re-request a tile
/// that finished loading either. The hole is silent and permanent, the same
/// end state as the image-cache adoption bug in rekey_safe_tile_provider.dart.
///
/// So the tile client re-types those failures on the way out. Nothing else
/// changes. [toString] returns the wrapped exception's text word for word,
/// because every consumer downstream classifies errors by text: net_error.dart
/// and its connectivity markers, TileStatusMonitor's 404 filter, and the debug
/// sheet's last error line. Each must keep seeing exactly what it saw before.
...
class _SurfacedFailureClient extends BaseClient {
  ...
  static Object _surface(Object error) {
    ...
    // monitor tells cancellations from failures by type. Preserve that type.
  ...
  Future<StreamedResponse> send(BaseRequest request) async {
    ...
    // from send(). Its wording is the one most likely to be mistaken for a
